...
function FeatureCard({ item }: { item: any }) {
  ...
  return (
    ...
    >
      ...
      {/* Card Title */}
      <h3 className="mb-7 text-[22px] font-bold leading-[1.2] tracking-[-0.04em] text-[#071515] transition-colors duration-500 group-hover:text-white dark:text-[#e8edee] dark:group-hover:text-[#0a1a18]">
        {item.title}
      </h3>

      {/* Impact Points */}
      {item.points && item.points.length > 0 && (
        <ul className="max-w-[345px] space-y-5">
          {item.points.map(
            (
              point: {
                metric: string;
              ...
              >
                {/* Bullet */}
                <span className="mt-[7px] h-1.5 w-1.5 flex-shrink-0 rounded-full bg-[#188b88] transition-colors duration-500 group-hover:bg-white dark:bg-[#4ecdc4] dark:group-hover:bg-[#0a1a18]" />

                {/* Text */}
                <div className="min-w-0">
                  <p className="text-[14px] font-bold leading-[1.45] tracking-[-0.025em] text-[#071515] transition-colors duration-500 group-hover:text-white dark:text-[#e8edee] dark:group-hover:text-[#0a1a18] lg:text-[15px]">
                    {point.metric}
                  </p>

                  <p className="mt-1 text-[13px] font-normal leading-[1.5] tracking-[-0.015em] text-[#6b7878] transition-colors duration-500 group-hover:text-white/75 dark:text-[#8f9c9b] dark:group-hover:text-[#0a1a18]/70 lg:text-[13.5px]">
                    {point.description}
                  </p>
                </div>
              </li>
